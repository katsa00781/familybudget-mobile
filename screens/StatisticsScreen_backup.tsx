...
const StatisticsScreen = () => {
  ...
  const calculateStatistics = (data: any[]) => {
    ...
    // Feldolgozzuk a bevásárlólistákat és az elemeit
    data.forEach(list => {
      const listTotal = list.total_amount || 0;
      totalSpent += listTotal;
      
      // Bolt statisztika
      const storeName = list.name || 'Ismeretlen bolt';
      const storeStats = storeData.get(storeName) || { total: 0, visits: new Set() };
      storeStats.total += listTotal;
      storeStats.visits.add(list.id);
      ...
      if (Array.isArray(items)) {
        items.forEach(item => {
          // Minden elemet hozzáadunk a feldolgozó tömbhöz
          allItems.push({
            ...item,
            listDate: list.date,
            storeName: list.name
          ...
          categoryStats.count += (item.quantity || 1);
          ...
          productStats.quantity += (item.quantity || 1);
          productStats.count += 1;
          productStats.price += (item.price || 0);
          if (new Date(list.date) > new Date(productStats.lastDate)) {
            productStats.lastDate = list.date;
          }
          productData.set(productName, productStats);
        });
        
        // Havi statisztikához hozzáadjuk a tételeket
        monthStats.total += listTotal;
        monthStats.count += items.length;
        monthlyData.set(monthKey, monthStats);
      }
    });
    
    // Beállítjuk az összesített értékeket
    setTotalSpent(totalSpent);
    setTotalItems(allItems.length);
    
    // Kategória statisztikák feldolgozása
    ...
    // Havi statisztikák feldolgozása
    ...
    // Termék statisztikák feldolgozása
    ...
    // Bolt statisztikák feldolgozása
