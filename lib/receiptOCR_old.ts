...
const preprocessImageForOCR = async (imageUri: string): Promise<string> => {
  try {
    ...
    console.log('📸 Kép előfeldolgozása a jobb OCR eredményért...');
    
    // Validálás - minimum kép méret ellenőrzése
    const imageInfo = await FileSystem.getInfoAsync(imageUri);
    if (imageInfo.exists && imageInfo.size && imageInfo.size < 10000) {
      console.warn('⚠️ Kép túl kicsi:', imageInfo.size, 'bytes');
    }
    
    return base64Image;
  } catch (error) {
    console.error('❌ Kép előfeldolgozási hiba:', error);
    throw error;
  }
};
...
export const processReceiptWithMindee = async (imageUri: string): Promise<ReceiptData> => {
  ...
  try {
    ...
    // Javított kép előfeldolgozás
    const base64Image = await preprocessImageForOCR(imageUri);
...
// 🎯 Intelligens kategória meghatározás magyar terméknevekhez
const determineCategory = (productName: string): string => {
  if (!productName) return 'Egyéb';
  
  const name = productName.toLowerCase();
  
  // 🥛 Tejtermékek - bővített lista
  const dairyKeywords = [
    'tej', 'sajt', 'túró', 'joghurt', 'kefir', 'vaj', 'margarin', 'tejföl', 'tejszín',
    'mozzarella', 'camembert', 'gouda', 'trappista', 'cottage', 'mascarpone', 'ricotta',
    'milk', 'cheese', 'yogurt', 'butter', 'cream'
  ];
  
  // 🍞 Pékáruk
  const bakeryKeywords = [
    'kenyér', 'kifli', 'zsemle', 'bagett', 'briós', 'croissant', 'kalács', 'rétes',
    'sütemény', 'torta', 'muffin', 'pite', 'bread', 'roll', 'cake', 'pastry'
  ];
  
  // 🥩 Hús és hal
  const meatKeywords = [
    'hús', 'csirke', 'sertés', 'marha', 'borjú', 'bárány', 'sonka', 'szalámi', 'kolbász',
    'virsli', 'bacon', 'hal', 'tonhal', 'lazac', 'pisztráng', 'tenger gyümölcse',
    'meat', 'chicken', 'pork', 'beef', 'ham', 'sausage', 'fish', 'salmon'
  ];
  
  // 🥬 Zöldség és gyümölcs
  const produceKeywords = [
    'alma', 'banán', 'narancs', 'citrom', 'eper', 'szőlő', 'barack', 'körte', 'szilva',
    'paradicsom', 'hagyma', 'krumpli', 'burgonya', 'répa', 'paprika', 'uborka', 'saláta',
    'káposzta', 'brokkoli', 'karfiol', 'spenót', 'petrezselyem', 'kapor', 'zeller',
    'apple', 'banana', 'orange', 'lemon', 'tomato', 'onion', 'potato', 'carrot'
  ];
  
  // 🍫 Édességek és snackek
  const snackKeywords = [
    'csokoládé', 'bonbon', 'cukor', 'méz', 'lekvár', 'nutella', 'chips', 'náci',
    'keksz', 'kréker', 'ostya', 'gumicukor', 'chocolate', 'candy', 'sugar', 'honey'
  ];
  
  // 🧴 Háztartási termékek
  const householdKeywords = [
    'mosószer', 'öblítő', 'mosogatószer', 'tisztítószer', 'wc papír', 'papírtörlő',
    'szemeteszsák', 'alufólia', 'fólia', 'detergent', 'soap', 'shampoo', 'toothpaste'
  ];
  
  // 🍻 Italok
  const beverageKeywords = [
    'víz', 'ásványvíz', 'üdítő', 'kóla', 'pepsi', 'fanta', 'sprite', 'sör', 'bor',
    'pálinka', 'whisky', 'vodka', 'rum', 'water', 'cola', 'beer', 'wine', 'juice'
  ];
  
  // Kategória keresési logika
  const categories = [
    { keywords: dairyKeywords, category: 'Tejtermékek' },
    { keywords: bakeryKeywords, category: 'Pékáruk' },
    { keywords: meatKeywords, category: 'Hús és hal' },
    { keywords: produceKeywords, category: 'Zöldség és gyümölcs' },
    { keywords: snackKeywords, category: 'Édességek' },
    { keywords: householdKeywords, category: 'Háztartás' },
    { keywords: beverageKeywords, category: 'Italok' }
  ];
  
  for (const { keywords, category } of categories) {
    if (keywords.some(keyword => name.includes(keyword))) {
      console.log(`🎯 Kategória találat: "${productName}" -> ${category}`);
      return category;
    }
  }
  
  console.log(`❓ Ismeretlen kategória: "${productName}" -> Egyéb`);
  return 'Egyéb';
};

// Segédfunkciók - Javított termék név feldolgozás
const cleanProductName = (name: string): string => {
  if (!name) return 'Ismeretlen termék';
  
  let cleaned = name.trim();
  
  // 🔧 OCR hibák javítása magyarul (gyakori félreolvasások)
  const ocrCorrections: { [key: string]: string } = {
    // Számok és betűk gyakori félreolvasása
    '0': 'O', '5': 'S', '1': 'I', '8': 'B', '6': 'G', '2': 'Z', '3': 'E',
    // Magyar termékek gyakori hibái
    'TEJF0L': 'TEJFÖL', 'K3NYÉR': 'KENYÉR', 'P4RADICSOM': 'PARADICSOM',
    'H4GYMA': 'HAGYMA', 'BURGONY4': 'BURGONYA', 'S4JT': 'SAJT',
    'J0GHURT': 'JOGHURT', 'CS1RKE': 'CSIRKE', '5ONKA': 'SONKA',
    'KOL8ÁSZ': 'KOLBÁSZ', 'SZ4LÁMI': 'SZALÁMI', 'P4PRIKA': 'PAPRIKA',
    'U80RKA': 'UBORKA', 'SÁLÁT4': 'SALÁTA', '4LMA': 'ALMA',
    'B4NÁN': 'BANÁN', 'N4RANCS': 'NARANCS', 'C1TROM': 'CITROM',
    'TÉJ': 'TEJ', 'V4J': 'VAJ', 'TO1ÁS': 'TOJÁS'
  };
  
  // OCR javítások alkalmazása
  ...
  cleaned = cleaned
    ...
    .replace(/\b(?:AKCIÓ|AKCIÓS|KEDVEZMÉNY|LEÁRAZ|SALE)\b/gi, '')     // Akciós címkék eltávolítása
    .replace(/[^a-zA-ZáéíóöőúüűÁÉÍÓÖŐÚÜŰ0-9\s\-]/g, '')  // Nem kívánt karakterek
  ...
  console.log(`🔧 Termék javítva: "${name}" -> "${cleaned}"`);
  return cleaned;
};
...
const parseHungarianReceiptText = (text: string): ReceiptData => {
  ...
  // 🛒 Termékek keresése - továbbfejlesztett algoritmus
  for (let i = 0; i < lines.length; i++) {
    ...
    const productPatterns = [
      // Standard: "TERMÉK NÉV    123,45 Ft"
      /^(.+?)\s+(\d{1,3}(?:[.,\s]\d{3})*(?:[.,]\d{1,2})?)\s*(?:Ft|HUF|EUR|€)?\s*$/i,
      // Mennyiségekkel: "ALMA 1kg x 450Ft = 450 Ft"
      /^(.+?)\s+\d+[.,]?\d*\s*(?:kg|g|dkg|db|l|dl|ml|csomag|csom|doboz|üveg)\s*x?\s*\d+\s*(?:Ft)?\s*=?\s*(\d{1,3}(?:[.,\s]\d{3})*(?:[.,]\d{1,2})?)\s*(?:Ft|HUF)?\s*$/i,
      // Akciós: "TERMÉK AKCIÓ 450 Ft"
      /^(.+?)\s+(?:AKCIÓ|AKCI[OÓ]S?|KEDVEZM[EÉ]NY|LEÁRAZ|%)\s*\d*%?\s*(\d{1,3}(?:[.,\s]\d{3})*(?:[.,]\d{1,2})?)\s*(?:Ft|HUF)?\s*$/i,
      // Dupla ár: "TERMÉK 500 Ft -> 450 Ft"
      /^(.+?)\s+\d+\s*Ft?\s*[-→>]\s*(\d{1,3}(?:[.,\s]\d{3})*(?:[.,]\d{1,2})?)\s*(?:Ft|HUF)?\s*$/i,
      // Egyszerű: "TERMÉK 450"
      /^(.+?)\s+(\d{1,3}(?:[.,\s]\d{3})*(?:[.,]\d{1,2})?)\s*$/i,
      // Pont és szóköz: "TERMÉK . . . . 450 Ft"
      /^(.+?)\s*[.\s-]+\s*(\d{1,3}(?:[.,\s]\d{3})*(?:[.,]\d{1,2})?)\s*(?:Ft|HUF)?\s*$/i
    ];
    ...
    for (const pattern of productPatterns) {
      ...
      if (match) {
        ...
        // Ár tisztítása és konvertálása
        const cleanPrice = cleanPriceString(priceStr);
        if (cleanPrice <= 0) continue; // Érvénytelen árak kihagyása
        
        // Termék név tisztítása
        productName = cleanProductName(productName);
        if (productName.length < 2) continue; // Túl rövid nevek kihagyása
        
        // Mennyiség és mértékegység kinyerése
        const quantity = extractQuantityFromLine(line) || 1;
        const unit = extractUnitFromDescription(line) || 'db';
        ...
        items.push(item);
        console.log(`✅ Termék hozzáadva: ${item.name} - ${item.price} Ft (${item.quantity} ${item.unit})`);
        foundMatch = true;
        break;
      }
    }
    
    // Ha nem találtunk egyezést, próbáljunk több soros elemzést
    if (!foundMatch && i < lines.length - 1) {
      const nextLine = lines[i + 1];
      const multiLineMatch = tryMultiLineProductParsing(line, nextLine);
      if (multiLineMatch) {
        items.push(multiLineMatch);
        console.log(`✅ Több soros termék: ${multiLineMatch.name} - ${multiLineMatch.price} Ft`);
        i++; // Kihagyjuk a következő sort, mert már feldolgoztuk
      }
    }
  }
